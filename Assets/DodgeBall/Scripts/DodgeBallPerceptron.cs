...
[System.Serializable]
public class DodgeBallTrainingSet
{
	public double[] input;
	public double output;
}

public class DodgeBallPerceptron : MonoBehaviour {
	...
	public void SendInput(double i1, double i2, double o)
	{
		...
		Debug.Log(result);
		if(result == 0) //se for zero o npc abaixa
		{
		...
		TrainingSet s = new TrainingSet(); //cria um novo training set baseado nos inputs e no resultado recebido
		s.input = new double[2] {i1,i2};
		s.output = o;
		ts.Add(s);
		Train(); //treina para levar em conta os novos dados
	}
